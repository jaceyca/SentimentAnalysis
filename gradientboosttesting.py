...
def main():
    ...
    # parameters = {'min_samples_split':np.linspace(2, 10, 5).astype(int), 'max_depth':[None, 2, 6, 10]}
    
    # clf = GridSearchCV(GradientBoostingClassifier(learning_rate=0.1, n_estimators=1000), parameters, cv=fold, n_jobs=-1, verbose=2)
    ...
    # gbclf = GradientBoostingClassifier(learning_rate=0.1, n_estimators=1000,
    ...
    #(learning_rate=0.1, n_estimators=1000, min_samples_split=2, max_depth=10): Gradient Boost error: 0.15075
    #(learning_rate=0.1, n_estimators=1000, min_samples_split=2, max_depth=10): Gradient Boost error: 0.192

    # gbsubmission = make_predictions(etclf, X_train_n, y_train, X_test_n)
    # save_data(etsubmission, "gradientboostingsubmission.txt")
